...
- Multi-channel fault framework integration (Opzione B):
  The two controller-side input channels are read from the framework's
  POST-INJECTION topics, NOT from the controllers directly. This lets
  the fault framework intercept and (when armed) corrupt these signals
  upstream of the bridge.
    ...
    admittance_controller --[/trajectory_ref_raw]--> fault_framework
        --[/trajectory_ref_post_inject]--> exo_bridge --[/trajectory_ref]--> trajectory_controller

  When the fault framework is disarmed (active_channel != 1,2 or
  fault_active=False), it forwards the *_raw input unchanged to the
  *_post_inject output, so the bridge sees the same data as before.
...
Layout of /exo_bridge/status (exoskeleton_safety_msgs/Float64ArrayStamped):
  ...
  data[8] = tau_raw           (pre-clamp torque from the controller, post-injection)
"""
import copy
...
# Channel names used by the watchdog (logical labels for log clarity,
# NOT topic names; updated to reflect the multi-channel framework topology).
CH_TORQUE  = 'torque_post_inject'
CH_TRAJ    = 'trajectory_ref_post_inject'
...
class ExoBridge(Node):
    def __init__(self):
        ...
        # ── Subscribers ──────────────────────────────────────────────
        # NOTE: torque and trajectory_ref are read from the fault framework
        # POST-INJECTION topics, not directly from the controllers.
        self.create_subscription(JointState,           '/joint_states',                     self._cb_js,  10)
        self.create_subscription(Float64ArrayStamped, '/trajectory_ref_post_inject',       self._cb_ref, 10)
        self.create_subscription(Float64Stamped,      '/torque_post_inject',               self._cb_tau, 10)
    ...
    def _safe_trajectory_ref(self) -> Optional[Float64ArrayStamped]:
        ...
        if len(ref.data) < 3:
            self.get_logger().warn('Malformed trajectory_ref_post_inject')
            return ref
    ...
    def _publish_status(self):
        ...
        # tau_raw is the pre-clamp torque from the controller (post-injection
        # if the framework is armed on Ch2; otherwise identical to the
