...
from launch.actions import ExecuteProcess, TimerAction, Shutdown  # <-- import aggiuntivi
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            executable='dynamics_paper',
            name='dynamics',
            output='screen',
            parameters=[{
                'dt': 0.001,                     # passo di integrazione (1ms o 0.5ms)
            }],
        ),
        ...
        # ── ROS 2 Bag recording (B, Bdot, theta) ──────────────────
        ExecuteProcess(
            cmd=['ros2', 'bag', 'record',
                 '/exo_dynamics/B',
                 '/exo_dynamics/Bdot',
        ...
        # ── Auto‑shutdown dopo 20 secondi ─────────────────────────
        TimerAction(
            period=20.0,
            actions=[Shutdown(reason='Simulation time expired')]
        ),
    ])