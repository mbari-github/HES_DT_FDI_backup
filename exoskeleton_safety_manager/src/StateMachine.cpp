...
void StateMachine::initialize()
{
  ...
  // New for Phase 1: SENSOR_DEGRADED_MODE service. Symmetric to
  // compliant_mode and torque_limit: data=true requests entry, data=false
  // requests release back to FAULT_MONITOR (only valid if currently in
  // SENSOR_DEGRADED_MODE — see callback for details).
  sensor_degraded_service_ = this->create_service<std_srvs::srv::SetBool>(
    "sensor_degraded_request",
    std::bind(&StateMachine::sensor_degraded_callback, this,
      std::placeholders::_1, std::placeholders::_2));
...
bool StateMachine::change_state(const std::string & state, const std::string & reason)
{
  try {
    ...
    else if (state == "SensorDegradedMode")   current_state_ = functional_safety::SafetyState::SENSOR_DEGRADED_MODE;
    else {
...
//
// SENSOR_DEGRADED_MODE: see safety_states.hpp for the full graph.
// In short: it is reachable from FAULT_MONITOR, COMPLIANT, TORQUE_LIMIT, and
// can return to FAULT_MONITOR or escalate to COMPLIANT, TORQUE_LIMIT, SAFE_STOP.
// It is NOT reachable from SAFE_STOP (the latch blocks anything but reset).
  ...
  functional_safety::SafetyState target_state) const
{
  ...
  switch (current_state_) {
    case functional_safety::SafetyState::FAULT_MONITOR:
      return target_state == functional_safety::SafetyState::SENSOR_DEGRADED_MODE ||
    ...
    case functional_safety::SafetyState::SENSOR_DEGRADED_MODE:
    ...
    case functional_safety::SafetyState::COMPLIANT_MODE:
      return target_state == functional_safety::SafetyState::TORQUE_LIMIT_MODE    ||
             target_state == functional_safety::SafetyState::SENSOR_DEGRADED_MODE ||
             target_state == functional_safety::SafetyState::SAFE_STOP             ||
             target_state == functional_safety::SafetyState::FAULT_MONITOR;
  ...
  functional_safety::SafetyState state) const
{
  switch (state) {
    ...
    case functional_safety::SafetyState::SENSOR_DEGRADED_MODE: return "SensorDegradedMode";
  ...
  functional_safety::SafetyState state) const
{
  switch (state) {
    ...
    case functional_safety::SafetyState::SENSOR_DEGRADED_MODE: return "SENSOR_DEGRADED_MODE";
    default: return "UNKNOWN";
  }
}

// state_to_id maps to the SafetyStatus.msg wire contract. Existing IDs (0..3)
// are preserved for backwards compatibility; SENSOR_DEGRADED_MODE was added
// as ID=4. Note that this ID is NOT the severity rank: see severity_rank()
// in safety_states.hpp for the ordering used by FDI arbitration.
int StateMachine::state_to_id(functional_safety::SafetyState state) const
{
  switch (state) {
    ...
    case functional_safety::SafetyState::SENSOR_DEGRADED_MODE: return 4;
    default: return -1;
  }
}
...
// SENSOR_DEGRADED_MODE service callback (Phase 1 addition).
//
// Pattern: identical to compliant_mode_callback and torque_limit_callback.
//   - data=true  → request entry to SENSOR_DEGRADED_MODE.
//                  This is the entry point used by the FDI supervisor
//                  (Phase 4+) when a sensor anomaly is detected with
//                  insufficient confidence to escalate to COMPLIANT_MODE.
//   - data=false → request release back to FAULT_MONITOR. Only valid if
//                  currently in SENSOR_DEGRADED_MODE; otherwise no-op.
//
// Note: just like compliant_mode_callback, this sets fault_present_ to true
// on entry. The FDI release should be issued only after the underlying
// anomaly has been confirmed to have cleared (e.g. multiple consecutive
// healthy windows from the autoencoder).
  ...
  std::shared_ptr<std_srvs::srv::SetBool::Response> response)
{
  if (request->data) {
    fault_present_ = true;
    const bool ok = request_state_change(
      functional_safety::SafetyState::SENSOR_DEGRADED_MODE, "sensor_degraded_request");
    response->success = ok;
    response->message = ok ?
      "SENSOR_DEGRADED_MODE activated" : "SENSOR_DEGRADED_MODE rejected";
    return;
  }

  if (current_state_ == functional_safety::SafetyState::SENSOR_DEGRADED_MODE) {
    const bool ok = request_state_change(
      functional_safety::SafetyState::FAULT_MONITOR, "sensor_degraded_release");
    if (ok) fault_present_ = false;
    response->success = ok;
    response->message = ok ?
      "Returned to FAULT_MONITOR from SENSOR_DEGRADED_MODE" : "Return rejected";
    return;
  }

  response->success = true;
  response->message = "SENSOR_DEGRADED_MODE already inactive";
}
...
//
// SENSOR_DEGRADED_MODE shares the bridge mode 'compliant' (mode_id=2)
// for the Phase 1 implementation. This avoids extending the bridge with
// a new mode while the FDI is still in development. When a dedicated
// 'sensor_degraded' bridge mode is added later, this mapping should be
// updated accordingly.
int StateMachine::expected_bridge_mode_id() const
{
  switch (current_state_) {
    ...
    case functional_safety::SafetyState::SAFE_STOP:            return 3;
    case functional_safety::SafetyState::SENSOR_DEGRADED_MODE: return 2;  // shares 'compliant' bridge mode
    default: return -1;
  }
}
