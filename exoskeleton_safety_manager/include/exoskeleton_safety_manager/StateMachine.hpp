 ...
 * SensorDegradedMode) via pluginlib and orchestrates transitions between them.
 *
 ...
 *
 * SENSOR_DEGRADED_MODE is part of the downgrade chain for completeness, but
 * the automatic downgrade logic does NOT manage entry/exit from it: that mode
 * is entered by an explicit request from the FDI supervisor (Phase 4+) and
 * released either by the FDI's own release request or by an explicit operator
 * action.
 *
 * ROS2 Parameters
 * ---------------
 ...
 *   /sensor_degraded_request    std_srvs/SetBool   — enter/exit SENSOR_DEGRADED_MODE
...
private:
  ...
  // New for Phase 1: SENSOR_DEGRADED_MODE service callback.
