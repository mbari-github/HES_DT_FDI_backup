...
This is the reference implementation. Compared to dynamics_stripped.py it adds:
  ...
  - **NEW** topics /exo_dynamics/B and /exo_dynamics/Bdot publishing the
    projection vector B = dq/dtheta and its time derivative.
...
class ExoReducedDynamicsWithHand(Node):
    ...
    def __init__(self) -> None:
        ...
        self.Bdot = np.zeros(self.nv)  # NEW: store Bdot for publishing
        ...
        # ── Per-pair closure residuals (one norm per frame pair + raw 3-D components) ──
        n_pairs = len(self.closure_frame_pairs)
        self.closure_res_per_pair = np.zeros(n_pairs)       # ‖eᵢ‖ for each pair
        self.closure_res_components = np.zeros(n_pairs * 3) # raw [x,y,z] for each pair
        ...
        # NEW: Publishers for the projection vector B and its derivative Bdot
        self.pub_B = self.create_publisher(Float64ArrayStamped, '/exo_dynamics/B', 10)
        self.pub_Bdot = self.create_publisher(Float64ArrayStamped, '/exo_dynamics/Bdot', 10)

        # NEW: Publisher for per-pair closure residuals
        # Layout: [total_norm, pair0_norm, pair1_norm, ...,
        #          pair0_ex, pair0_ey, pair0_ez, pair1_ex, ...]
        self.pub_closure_res = self.create_publisher(
            Float64ArrayStamped, '/exo_dynamics/closure_residuals', 10
        )
    ...
    # ============================================================
    # PER-PAIR CLOSURE RESIDUALS
    # ============================================================

    def _compute_per_pair_residuals(self, q: np.ndarray) -> None:
        """Compute ‖eᵢ‖ and raw components for each closure frame pair at q.
        Requires forwardKinematics + updateFramePlacements to have been called."""
        pin.forwardKinematics(self.model, self.data, q)
        pin.updateFramePlacements(self.model, self.data)
        for i, (ida, idb) in enumerate(self.closure_frame_pairs):
            e = self.data.oMf[ida].translation - self.data.oMf[idb].translation
            self.closure_res_per_pair[i] = float(np.linalg.norm(e))
            self.closure_res_components[3 * i: 3 * i + 3] = e
    ...
    def step(self) -> None:
        ...
        # Per-pair closure residuals at the solved configuration
        self._compute_per_pair_residuals(self.q)

        # NEW: compute and store Bdot for publishing
        self.Bdot = (self.B - self.B_prev) / self.dt

        Bdot = self.Bdot  # alias for use in dynamics
    ...
    def publish(self) -> None:
        ...
        # --- NEW: publish B and Bdot vectors ---
        now = self.get_clock().now().to_msg()
        
        msg_B = Float64ArrayStamped()
        msg_B.header.stamp = now
        msg_B.data = self.B.tolist()
        self.pub_B.publish(msg_B)

        msg_Bdot = Float64ArrayStamped()
        msg_Bdot.header.stamp = now
        msg_Bdot.data = self.Bdot.tolist()
        self.pub_Bdot.publish(msg_Bdot)

        # --- /exo_dynamics/closure_residuals ---
        # Layout: [total_norm, pair0_norm, ..., pairN_norm,
        #          pair0_ex, pair0_ey, pair0_ez, pair1_ex, ...]
        # Pair names: frame_AC_end↔frame_rod_end, frame_AC_end_2↔frame_BC_end,
        #             frame_CE_end↔frame_BC_end_2, frame_CE_end_2↔slider_t
        msg_cr = Float64ArrayStamped()
        msg_cr.header.stamp = now
        msg_cr.data = (
            [float(self.last_closure_norm if np.isfinite(self.last_closure_norm) else -1.0)]
            + self.closure_res_per_pair.tolist()
            + self.closure_res_components.tolist()
        )
        self.pub_closure_res.publish(msg_cr)
